...
### 10. WeaveMind / Weft Programming Language
- **Source:** weavemind.ai; github.com/WeaveMindAI/weft; Hacker News post by Quentin Feuillade--Montixi
- **Date:** Announced February 9, 2026; open-sourced April 15, 2026
- **What it covers:**
  - Domain-specific programming language (Weft) for building AI systems
  - Visual node-based graph model where humans and AI are first-class nodes
  - Typed inputs/outputs on nodes (generics, unions, type variables)
  - Sequential, parallel, and conditional execution patterns (implicit in node graph)
  - Compilation of Weft programs to native Rust via Restate durable execution engine
  - TOML-based manifest (WeftOS) for lifecycle (install/start/stop) and agent spawning
  - Human-in-the-loop: programs pause for human input, resume after days
  - One deployable artifact across platforms (via Rust cross-compilation)
  - O'Saasy license (MIT + SaaS restriction)
- **What it DOESN'T cover:**
  - **No declarative AI flow manifest with typed phases and NL prompt templates** — Weft is a compiled language; phase definitions are code nodes, not natural language templates usable as validation reference specifications
  - **No formal lifecycle state machine** — no published/validated/graduated/compiled progression with algorithmic entry conditions based on consensus validation proofs
  - **No shadow validation or BFT consensus** — compilation is a standard language compiler, not trust-weighted consensus over behavioral divergence
  - **No token economics or domain-based access tiering** — no TRUST token, no domain-specific thresholds, no meritocratic access model
  - **No multi-party verifiable collaboration record** — no session synthesis fingerprinting, no public verification URL
  - **No content-based phase completion detection** — Weft uses explicit graph transitions, not AI output content analysis
  - **No decentralized governance** — centralized SaaS service with O'Saasy license restrictions
  - Compilation to Rust via standard language compiler ≠ autonomous compilation of graduated flows via consensus validation corpus (the SkillChain CIP compilation mechanism requires statistical proof of output determinism from validation corpus before compilation is triggered)
- **Claims affected:**
  - Claims 31, 32 (manifest format with execution patterns) — moderate risk due to TOML manifest + execution patterns overlap
  - Claims 23–28 of Parent (Verified Fix Engine) — Weft's compilation pipeline is superficially similar but is a language compiler, not a multi-candidate tournament fix engine
  - CIP Claim 1 (Autonomous Compilation) — Weft compiles to deterministic code, but trigger mechanism (100+ consensus proofs at 95% similarity) is entirely absent
- **Priority note:** Family F patent draft is dated 2026-03-31. WeaveMind was announced February 9, 2026, which is prior to the draft date. However: (a) the invention was reduced to practice before Feb 2026 (code exists in the repository with earlier commits); (b) WeaveMind does not disclose the specific claimed innovations (lifecycle state machine, consensus-triggered compilation, token economics, conversational phase execution). US provisional priority date will control; attorney should confirm filing date establishes priority over Feb 9, 2026.
- **Relevance:** MEDIUM for Claims 31-32 (manifest + execution patterns); LOW for all other claims. Key differentiator: Weft is a compiled programming language targeting a fixed Rust/Restate stack; SkillChain defines a declarative interoperability standard with decentralized consensus validation, meritocratic token economics, and autonomous compilation triggered by statistical proof of output determinism. The spirit and purpose are different: Weft is for building AI systems faster; SkillChain is for validating, standardizing, and economically governing AI-guided flows in a decentralized network.

### 11. Google A2A Protocol and GOAT Universal Adapter
...
| 17 | Self-contained skill archive (metadata, definition, implementation, tests, reference outputs, proofs, signatures) | LOW | Anthropic SKILL.md exists for format; SkillChain's archive with validation proofs and cryptographic signatures for decentralized validation is novel. WeaveMind's TOML manifest covers configuration only, not validation proofs. |
| 18 | Platform adapters (structured markup / NL instructions / executable code) | MEDIUM | Cross-platform conversion exists (A2A, GOAT, WeaveMind Rust cross-compilation); specific three-mode translation for validated skill archives with decentralized proof chain is novel. |
