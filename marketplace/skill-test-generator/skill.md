...
Analyzes a SkillChain skill definition (skill.md + manifest.json), extracts its execution phases, inputs, outputs, and quality gates, then generates comprehensive test cases that cover all phases, edge cases, and error handling paths, validates test coverage, and predicts shadow validation success probability.

## Execution Pattern: Phase Pipeline

## Inputs
- skill_md_content: string -- Full text content of the skill.md file to generate tests for
- manifest_json: object -- The skill's manifest.json (name, inputs, outputs, execution_pattern, domain)
...
### Phase 1: ANALYZE -- Extract Skill Structure
**Entry criteria:** skill.md content is provided and non-empty.
**Actions:**
1. Parse the skill.md to extract:
   ...
   - **Reference section:** Domain knowledge the skill draws on
2. From the manifest, extract:
   - Domain, tags, input/output field names
   - Execution pattern (cross-validate with skill.md declaration)
...
4. Identify the skill's "core competency" -- the primary thing it does that must be tested most rigorously.

**Output:** Structured skill anatomy: phases with actions/outputs, quality gates, error handlers, core competency identification.
**Quality gate:** Every phase in the skill.md is identified and extracted. At least one testable behavior per phase is identified. The execution pattern matches between manifest and skill.md.

### Phase 2: GENERATE -- Create Test Cases
**Entry criteria:** Skill anatomy extracted with testable behaviors identified.
**Actions:**
...
2. Generate one edge case test:
   - **Minimal input:** What happens with the absolute minimum viable input? (Tests whether the skill handles sparse data gracefully.)
...
4. For each generated test case:
   - **Input:** A natural-language prompt that a user would realistically ask this skill.
   - **Expected keywords:** 6-12 terms that must appear in a correct response. Guidelines:
     ...
     - Include action verbs from the skill's defined actions ("classify," "score," "calculate")
     ...
     - Include at least 2 keywords unique to the correct execution path (wouldn't appear if the skill skipped phases)
5. Generate at minimum 5 test cases total. More for complex skills (1 per phase + 1 edge + 1 integration as minimum).
...
**Actions:**
1. Evaluate skill quality factors that predict shadow validation success:
...
2. Calculate predicted success probability:
   - Score 0-40: <30% validation success (skill needs major rework)
   - Score 41-60: 30-50% success (skill needs improvement in weakest areas)
   - Score 61-75: 50-70% success (solid skill, minor improvements recommended)
   - Score 76-90: 70-85% success (well-structured skill)
   - Score 91-100: 85-95% success (excellent skill, high confidence)
3. Generate specific improvement recommendations for the lowest-scoring factors.

**Output:** Skill quality score breakdown, predicted validation success %, specific improvement recommendations.
...
|---|---|---|
| ANALYZE | skill.md is malformed or doesn't follow execution standard | Adjust -- extract what's possible, flag structural issues as part of quality scoring |
...
| SCORE | Insufficient data for prediction (very short skill.md) | Adjust -- provide prediction with high uncertainty range, note limited confidence |
...
2. **Realistic inputs:** Test inputs should sound like real user prompts, not synthetic test data. This tests the skill as it will actually be used.
...
4. **Edge cases matter:** The difference between a good skill and a great skill is how it handles unusual inputs, missing data, and error conditions.
...
- **Include:** Action verbs from the skill's defined actions ("classify," "rank," "calculate," "generate")
...
Shadow validation runs a skill 5 times with the same input and checks:
...
Skills fail shadow validation primarily because:
- Vague phases produce different outputs on each run
- Missing quality gates allow garbage to propagate
- Undefined exit criteria mean the skill sometimes stops early
...
- Common coverage gaps by skill domain
