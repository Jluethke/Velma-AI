...
Convert a working ad-hoc prompt that produces good results into a publishable FlowFabric flow. Analyze the prompt's implicit structure, detect its execution pattern (ORPA or Phase Pipeline), extract inputs/outputs/phases/quality gates, generate a complete .skillpack, and validate through shadow execution.
...
PHASE 4: GENERATE   --> Create a complete .skillpack (manifest, flow.md, test cases)
PHASE 5: VALIDATE   --> Run shadow validation on the generated flow
```
...
- `generated_skillpack`: object -- Complete flow files: manifest.json, flow.md, tests/test_cases.json, examples/example_1.md, provenance.json
...
3. **Gather examples (if not provided):**
   ...
   - These become the test cases for the generated flow
...
5. **Identify implicit knowledge:**
   ...
   - This knowledge becomes the Reference section of the flow
...
### Phase 3: STRUCTURE -- Build the Flow Skeleton
...
4. **Define exit criteria:**
   - When is the flow DONE? (extract from prompt's completion signals)
...
6. **Extract reference material:**
   ...
   - Organize into the Reference section of flow.md
...
7. **Design state persistence:**
   - What should the flow remember between runs?
   - Patterns seen, configurations learned, history of inputs/outputs
   - This is what differentiates a flow from a one-shot prompt

**Output:** Complete flow skeleton: inputs, outputs, phases with full structure, exit criteria, error handling, reference material, state persistence design.
...
**Entry criteria:** Flow skeleton is complete and validated.
...
2. **Generate flow.md:**
   - Follow the Flow.md Template from the SkillChain Execution Standard exactly
...
3. **Generate tests/test_cases.json:**
   ...
   - Test cases validate that the flow's execution is consistent

4. **Generate examples/example_1.md:**
   - Create a complete worked example showing the flow executing on a real input
   - Walk through each phase with specific intermediate outputs
   - Show the final output in full
   - This is the "proof of execution" that demonstrates the flow works

5. **Generate provenance.json:**
   - Creator, creator node, timestamp, origin
   - Add: `"converted_from": "prompt"` to indicate this is a prompt-derived flow
...
**Quality gate:** manifest.json passes schema validation. flow.md follows the execution standard template. 5 test cases cover diverse scenarios. Example walks through every phase. Provenance is complete.
...
1. **Run 5 independent shadow executions:**
   - Take each test case input and execute the flow against it
...
2. **Score consistency:**
   ...
   - Overall flow passes if >= 4 out of 5 test cases pass (80%)
...
- The generated flow is demonstrably more consistent than the raw prompt
...
| ANALYZE | Prompt doesn't fit either execution pattern | **Adjust** -- decompose the prompt into sub-prompts, each converting to a separate flow, linked by composition |
...
- **Domain templates**: pre-built flow skeletons for common domains (development, writing, analysis)
