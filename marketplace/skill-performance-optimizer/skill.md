...
Reads a skill's execution history and validation results to identify performance bottlenecks -- slow phases, frequent failures, inconsistent outputs, and test coverage gaps -- then produces specific, implementable optimization recommendations with predicted impact on validation success rate and execution quality.

## Execution Pattern: ORPA Loop

## Inputs
- skill_md_content: string -- The skill.md being optimized (current version)
...
- recommended_changes: array -- Specific modifications to skill.md with predicted impact on performance metrics
...
3. If only skill.md is available (no history):
...
4. Cross-reference: do structural weaknesses in skill.md correlate with observed failures in execution history?
...
1. For each identified performance issue, determine root cause:
   ...
   - **Inconsistent outputs:** Are actions vague (different interpretations each run)? Is output format undefined? Are there multiple valid approaches and the skill doesn't specify which to use?
   - **Low keyword match:** Are expected keywords too specific (synonyms fail)? Is the skill not covering the concept the keywords test? Are keywords from phases the skill skips?
2. Classify root causes by type:
   - **Structural:** The skill.md design is flawed (missing phases, wrong pattern, etc.)
   - **Prompt quality:** Actions are described ambiguously, producing variable outputs
   - **Test misalignment:** Tests don't match what the skill actually does well
   - **Scope:** The skill tries to do too much in one phase or one run
   - **Dependency:** The skill depends on external data or capabilities it doesn't always have
...
1. For each root cause, generate a specific, implementable recommendation:
   ...
   **For test misalignment:**
   - "Replace keyword 'analysis' (too generic, matches any response) with 'Eisenhower quadrant' (specific to this skill's methodology)."
...
**Quality gate:** Every recommendation includes: (1) exactly what to change in skill.md (with before/after text), (2) which performance metric it targets, (3) predicted improvement magnitude. No recommendation is vague ("improve the prompts").
...
2. If this is a re-entry (optimizing a previously optimized skill):
...
3. Record optimization patterns for reuse:
   - "Adding output format constraints improved consistency by X% across Y skills" -- this becomes a general optimization heuristic.

**Output:** Complete optimization report with summary, quick wins, full recommendations, implementation order, monitoring plan.
**Quality gate:** Report addresses the skill's top 3 performance issues. Every recommendation has predicted impact. Quick wins are genuinely quick (<15 min to implement). Monitoring plan specifies what to measure and when.
...
| ACT | Skill is already high-performing (>90% validation pass rate) | Skip major recommendations -- focus on marginal improvements and maintenance |

## Reference

### Performance Optimization Hierarchy
Fix in this order (each level builds on the previous):
1. **Structure:** Ensure phases, quality gates, and exit criteria exist. No point optimizing a skill with missing foundations.
...
### State Persistence
Tracks over time:
- Per-skill performance metrics before and after each optimization
- Which recommendation types had the most impact (building an optimization playbook)
- Common root causes by skill domain
