...
Reads a FlowFabric flow definition and evaluates it against the Execution Standard, producing a 0-100 quality score across eight dimensions with severity-ranked issues and specific, actionable improvement suggestions.

## Execution Pattern: ORPA Loop

## Inputs
- skill_md_content: string -- Full text of the flow.md file to evaluate
- manifest_json: object -- The flow's manifest.json for cross-validation
...
**Entry criteria:** flow.md content provided.
**Actions:**
1. Parse the flow.md for all Execution Standard elements:
...
2. Cross-validate with manifest.json:
   - Does the execution_pattern field match the flow.md declaration?
   - Are all manifest inputs referenced in flow.md?
...
**Quality gate:** flow.md has been fully parsed. All 7 required element categories have been checked.
...
**Quality gate:** All 8 dimensions have a score. Rationale cites specific evidence from the flow.md (quotes or references to specific sections).
...
1. Compile all identified issues and classify by severity:
   ...
   - **Minor (polish):** Missing error handling for edge cases, reference section could be deeper, input constraints not specified. Nice to have but flow can function without.
...
**Quality gate:** Every scored deduction (where the flow lost points) has a corresponding improvement suggestion. Suggestions are specific enough to implement without further guidance.
...
1. Format the quality report:
   ...
   - **Benchmark comparison:** How this flow compares to the median quality on SkillChain (if data available)
...
|---|---|---|
| OBSERVE | flow.md is empty or has no recognizable structure | Abort -- return score of 0 with message "flow.md does not follow any recognized format" |
| OBSERVE | manifest.json missing | Adjust -- score based on flow.md alone, note that cross-validation was skipped |
...
### Common Anti-Patterns in Flow Design
1. **The Knowledge Dump:** Lots of reference material but no execution structure. The flow knows things but doesn't DO things consistently.
...
5. **The Eternal Loop:** ORPA flow with no exit criteria -- keeps looping without a clear completion condition.
...
### State Persistence
Tracks over time:
- Quality scores per flow per version (trend analysis)
- Most common issues by domain (are "meta" flows better structured than "productivity" flows?)
