...
Takes an existing skill.md and makes it better. Identifies weak quality gates, missing error handling, vague instructions, untested edge cases, and overly complex phases. Produces a revised skill.md with specific improvements and a before/after comparison. Fork this to create domain-specific skill auditors.

## Execution Pattern: ORPA Loop

```
OBSERVE  --> Read the skill and catalog every section
REASON   --> Score each section against quality criteria
PLAN     --> Prioritize improvements by impact
ACT      --> Rewrite weak sections and produce the improved skill
LOOP     --> Re-score to confirm improvement; exit when all sections pass
```

## Inputs

- `skill_md`: string -- The complete contents of the skill.md to improve.
...
- `domain_context`: string (optional) -- Domain the skill operates in, for domain-specific quality checks.

## Outputs

- `audit_report`: object -- Section-by-section scoring with specific issues found.
- `improved_skill`: string -- The revised skill.md with all improvements applied.
...
### OBSERVE -- Catalog the Skill

**Actions:**

1. Parse the skill.md into sections: title, description, execution pattern, inputs, outputs, phases, exit criteria, error handling, reference, state persistence.
...
1. **Description quality (0-10):** Does the one-liner describe a concrete procedure? Does it say what the skill DOES, not what it IS? Deduct for: passive voice (-1), no action verb (-2), longer than 2 sentences (-1), uses "helps" or "assists" (-2).
...
7. **Exit criteria clarity (0-10):** Exit criteria are concrete and testable. The skill is unambiguously DONE when criteria are met. Deduct for: criteria using "complete" without definition (-2), fewer than 3 exit criteria (-1 per missing), criteria that mirror a single phase gate (-1).
...
6. Assemble the complete improved skill.md.

**Output:** Improved skill.md and detailed change log.

**Quality gate:** Every issue from the PLAN phase has a corresponding entry in the change log. The improved skill.md parses into valid sections.

---

### LOOP -- Verify Improvement

Re-run the REASON scoring on the improved skill.md. Compare before and after scores. If any dimension is still below 6/10, loop back to PLAN for that dimension. Exit when all dimensions score 6+ or two improvement loops have been completed.
...
- The improved skill.md follows SkillChain execution standard format
...
|---|---|---|
| OBSERVE | Input is not valid skill.md format (no recognizable sections) | **Abort** -- the input is not a skill. Suggest using skill-from-workflow or prompt-to-skill-converter first |
| REASON | Skill scores 9+ on all dimensions already | **Abort** -- skill is already high quality. Return the audit report with a note that no improvements are needed |
| PLAN | More than 20 issues identified | **Adjust** -- focus on the top 10 highest-impact issues only to avoid a complete rewrite |
| ACT | Rewriting a section changes the skill's fundamental purpose | **Adjust** -- flag the section as needing author review rather than rewriting it |
