...
echo   "$settingsPath = Join-Path $env:USERPROFILE '.claude\settings.json'; " ^
echo   "if ^(Test-Path $settingsPath^) { " ^
echo   "  $settings = Get-Content $settingsPath -Raw ^| ConvertFrom-Json; " ^
echo   "  if ^($settings.mcpServers.PSObject.Properties['skillchain']^) { " ^
echo   "    $settings.mcpServers.PSObject.Properties.Remove^('skillchain'^); " ^
echo   "    $settings ^| ConvertTo-Json -Depth 10 ^| Set-Content $settingsPath -Encoding UTF8; " ^
echo   "    Write-Host '  Removed from Claude Code settings.'; " ^
echo   "  } else { " ^
echo   "    Write-Host '  Not found in Claude Code settings.'; " ^
echo   "  } " ^
echo   "} else { " ^
echo   "  Write-Host '  Claude settings.json not found.'; " ^
echo   "}"
echo echo   [2/5] Removing PATH entry...
echo powershell -NoProfile -ExecutionPolicy Bypass -Command ^
echo   "$binDir = Join-Path $env:USERPROFILE '.skillchain\bin'; " ^
echo   "$currentPath = [Environment]::GetEnvironmentVariable^('Path', 'User'^); " ^
echo   "if ^($currentPath -and $currentPath -like \"*$binDir*\"^) { " ^
echo   "  $newPath = ^($currentPath.Split^(';'^) ^| Where-Object { $_ -ne $binDir }^) -join ';'; " ^
echo   "  [Environment]::SetEnvironmentVariable^('Path', $newPath, 'User'^); " ^
echo   "  Write-Host '  Removed from PATH.'; " ^
echo   "} else { " ^
echo   "  Write-Host '  Not on PATH.'; " ^
echo   "}"
echo echo   [3/5] Removing SkillChain directory...
echo set "SC_DIR=%%USERPROFILE%%\.skillchain"
echo if exist "%%SC_DIR%%" ^(
echo     rmdir /s /q "%%SC_DIR%%"
echo     echo   Removed %%SC_DIR%%
echo ^) else ^(
