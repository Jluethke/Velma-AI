...
contract TrustTokenTest is Test {
    ...
    function setUp() public {
        vm.prank(admin);
        token = new TrustToken(admin, wayfinder, treasury);
    }
    ...
    function test_MintRevertCapExceeded() public {
        ...
        vm.prank(admin);
        vm.expectRevert("TrustToken: cap exceeded");
    ...
    function test_ClaimVestedRevertNoVesting() public {
        vm.prank(user1);
        vm.expectRevert("TrustToken: no vesting");
        token.claimVested();
    }
    ...
    function test_ConstructorRevertZeroAdmin() public {
        vm.expectRevert("TrustToken: zero admin");
        new TrustToken(address(0), wayfinder, treasury);
    }

    function test_ConstructorRevertZeroWayfinder() public {
        vm.expectRevert("TrustToken: zero wayfinder");
        new TrustToken(admin, address(0), treasury);
    }

    function test_ConstructorRevertZeroTreasury() public {
        vm.expectRevert("TrustToken: zero treasury");
        new TrustToken(admin, wayfinder, address(0));
    }
}
