...
/**
 * @title TrustToken
 * @notice ERC-20 token for the FlowFabric network — "TRUST" (TRUST).
 ...
 */
contract TrustToken is ERC20, ERC20Burnable, ERC20Permit, AccessControl {
    ...
    constructor(
        ...
        address wayfinder,
        address daoTreasury
    ) ERC20("TRUST", "TRUST") ERC20Permit("TRUST") {
        require(admin != address(0), "TrustToken: zero admin");
        require(wayfinder != address(0), "TrustToken: zero wayfinder");
        require(daoTreasury != address(0), "TrustToken: zero treasury");
    ...
    function mint(address to, uint256 amount) external onlyRole(MINTER_ROLE) {
        require(totalSupply() + amount <= MAX_SUPPLY, "TrustToken: cap exceeded");
        _mint(to, amount);
    }
    ...
    function claimVested() external {
        ...
        require(vs.total > 0, "TrustToken: no vesting");
