...
def _save_settings(settings: Dict[str, Any]) -> None:
    ...
    Creates a .bak backup before overwriting so that a crash or
    interrupted write never loses the user's previous config.
    """
    _SETTINGS_PATH.parent.mkdir(parents=True, exist_ok=True)

    # Backup existing file before overwriting
    if _SETTINGS_PATH.exists():
        backup_path = _SETTINGS_PATH.with_suffix(".json.bak")
        try:
            import shutil
    ...
    try:
        ...
        raise RuntimeError(f"Generated invalid JSON, aborting save: {exc}") from exc

    _SETTINGS_PATH.write_text(new_content, encoding="utf-8")
